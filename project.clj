(defproject cider/cider-nrepl "0.21.2-SNAPSHOT"
  ...
  :mranderson {:overrides   {[mvxcvi/puget fipp] [fipp "0.6.15"]}; only takes effect if shadowing-only is false, which case [fipp "0.6.15"] can be removed from th dependencies vector
               :expositions [[mvxcvi/puget fipp]]; only takes effect if shadowing-only is false
               :shadowing-only true}
